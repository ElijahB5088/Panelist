...
fun DiscoverScreen(
    ...
    var results by remember { mutableStateOf<List<MetadataGroup>>(emptyList()) }
    ...
    LaunchedEffect(query, repository) {
        ...
        if (repository == null) {
            ...
            errorMessage = null
            runCatching {
                if (query.trim().length < 2) repository.featured("discover")
    ...
    Column(modifier = Modifier.fillMaxSize().padding(horizontal = 20.dp, vertical = 24.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
        ...
        Row(
            ...
            listOf(
                "All" to "All sources",
                "comicvine" to "Comic Vine",
                "gcd" to "Grand Comics Database",
                "openlibrary" to "Open Library",
                "anilist" to "AniList"
        ...
        when {
            ...
            errorMessage != null -> {
                ...
                Button(onClick = { query = query.trim() }) { Text("Try again") }
            }
        ...
        if (!isLoading && errorMessage == null && filteredResults.isEmpty()) {
            Surface(shape = RoundedCornerShape(18.dp), tonalElevation = 2.dp) {
                Text(
                    if (query.isBlank()) "Featured titles will appear here when the server has catalog data."
                    else "No matching titles yet. Try a broader search.",
